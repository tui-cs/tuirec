...
| agg distribution | Bundle pinned `agg v1.8.1` in release archives and support source-build prerequisites | Release archives include a sibling `agg` binary that tuirec auto-detects before PATH. `go install`/local builds still require `agg` on PATH or `--agg-path`. Upstream does not publish a native Windows ARM64 asset for `v1.8.1`; Windows ARM64 archives include the x64 Windows binary for OS emulation, or users can build `agg` from source and pass `--agg-path` |
...
4. **FR-4: Render the cast file to an animated GIF**
   - Invoke pinned `agg v1.8.1` (prerequisite; see Decisions + CI).
...
| 8 | Windows full-GIF integration | `go test -tags integration ./...` green on ubuntu + macOS + windows: CI installs pinned `agg v1.8.1` on Windows and runs the real CLI `internal/testapp` -> validated GIF path there too. | Same canonical CLI demo on Windows, using repo-local `tools\agg.exe` or `agg` on PATH. |
...
2. GoReleaser (via `.github/workflows/release.yml`) downloads pinned `agg v1.8.1`, then builds for linux/darwin/windows × amd64/arm64
...
- `.github/workflows/ci.yml` — already present. Pins **Go 1.22**, runs
  ...
  on ubuntu, and an integration job that installs **pinned `agg v1.8.1`**
...
| agg not available on all platforms | Pinned `agg v1.8.1`, installed per-OS in CI and bundled into release archives; source-build prerequisite behavior is documented |
...
3. PTY recording and the full cast→GIF pipeline work on Linux, macOS,
   **and Windows** (ConPTY)
4. Release archives include `tuirec` plus pinned `agg v1.8.1`; source builds require `agg` separately
