...
4. **FR-4: Render the cast file to an animated GIF**
   - Invoke pinned `agg v1.11.0-sixel` (prerequisite; see Decisions + CI).
...
| 8 | Windows full-GIF integration | `go test -tags integration ./...` green on ubuntu + macOS + windows: CI installs pinned `agg v1.11.0-sixel` on Windows and runs the real CLI `internal/testapp` -> validated GIF path there too. | Same canonical CLI demo on Windows, using repo-local `tools\agg.exe` or `agg` on PATH. |
...
2. GoReleaser (via `.github/workflows/release.yml`) downloads pinned `agg v1.11.0-sixel`, then builds for linux/darwin/windows × amd64/arm64
...
- `.github/workflows/ci.yml` — already present. Pins **Go 1.22**, runs
  ...
  on ubuntu, and an integration job that installs **pinned `agg v1.11.0-sixel`**
...
| agg not available on all platforms | Pinned `agg v1.11.0-sixel`, installed per-OS in CI and bundled into release archives; source-build prerequisite behavior is documented |
...
3. PTY recording and the full cast→GIF pipeline work on Linux, macOS,
   **and Windows** (ConPTY)
4. Release archives include `tuirec` plus pinned `agg v1.11.0-sixel`; source builds require `agg` separately
