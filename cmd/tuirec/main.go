...
func newRootCommand(options cliOptions) *cobra.Command {
	root := &cobra.Command{
		Use:           "tuirec",
		Short:         "Record terminal apps and produce GIFs or PNG snapshots",
		Long:          "tuirec records terminal application sessions and renders them as animated GIFs or still PNG snapshots.",
		SilenceUsage:  true,
		SilenceErrors: true,
	}
	...
	root.AddCommand(newSnapshotCommand(options))
...
pacing to stderr.`,
	...
	cmd.Flags().BoolVar(&flags.config.Trim, "trim", flags.config.Trim, "Rebase cast to first visible output and trim alternate-screen-exit postroll")
...
func newSnapshotCommand(options cliOptions) *cobra.Command {
	flags := defaultSnapshotFlags()
	cmd := &cobra.Command{
		Use:   "snapshot",
		Short: "Capture a still PNG snapshot of a terminal app",
		Long: `Capture a still PNG snapshot of a terminal app.

This command records a cast using the same PTY pipeline as "record", then
...
or a timeline point in milliseconds (at:<ms>).`,
	...
	cmd.Flags().IntVar(&flags.inputDelayMS, "input-delay", flags.inputDelayMS, "Milliseconds to wait before playing the keystroke script")
	cmd.Flags().IntVar(&flags.startupDelayMS, "startup-delay", flags.startupDelayMS, "Milliseconds to wait after starting the target before copying output and key input")
	cmd.Flags().StringVar(&flags.config.ShowCommand, "show-command", "", "Synthetic shell command line to type before the target starts")
	cmd.Flags().IntVar(&flags.showCommandDelayMS, "show-command-delay", flags.showCommandDelayMS, "Milliseconds between typed show-command characters")
	cmd.Flags().IntVar(&flags.showCommandHoldMS, "show-command-hold", flags.showCommandHoldMS, "Milliseconds to hold after show-command Enter before target starts")
	cmd.Flags().IntVar(&flags.config.Size.Cols, "cols", flags.config.Size.Cols, "Terminal columns")
	cmd.Flags().IntVar(&flags.config.Size.Rows, "rows", flags.config.Size.Rows, "Terminal rows")
	...
	cmd.Flags().StringVar(&flags.pointerStyle, "pointer-style", "●", "Unicode character to display as the mouse pointer indicator")
	cmd.Flags().BoolVar(&flags.config.Inline, "inline", false, "Record an inline-mode app: skip alternate screen so prompt and app output share the normal screen buffer")
	cmd.Flags().BoolVar(&flags.openGIF, "open", false, "Open the PNG in the default viewer after capture")
	cmd.Flags().BoolVar(&flags.copyPath, "copy", false, "Copy the PNG file path to the system clipboard after capture")
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			doc := openCLIDocument{
				...
				Info: openCLIInfo{
					Title:       "tuirec",
					Version:     version,
					Description: "Cross-platform CLI that records terminal apps and produces animated GIFs or still PNG snapshots.",
				},
			}
...
func defaultSnapshotFlags() *recordFlags {
	flags := defaultRecordFlags()
	flags.config.Output = "snapshot.png"
	flags.config.Keystrokes = "wait:1500"
	flags.frame = "last"
	return flags
}
...
func runSnapshot(ctx context.Context, options cliOptions, flags *recordFlags) error {
	if flags.config.Binary == "" {
		return usageError(fmt.Errorf("--binary is required"))
	}
	...
	frame, err := pngrenderer.ParseFrameSelection(flags.frame)
	if err != nil {
		return usageError(err)
	}
	...
	config.Verbose = flags.verbosity == "high"
	config.LogWriter = options.stderr
	config.Renderer = pngrenderer.Renderer{Frame: frame}

	if flags.name != "" {
		if !flags.outputExplicit {
			config.Output = filepath.Join("artifacts", flags.name+".png")
		}
		if !flags.castOutputExplicit {
			config.CastOutput = filepath.Join("artifacts", flags.name+".cast")
		}
	...
	if flags.verbosity != "quiet" {
		fmt.Fprintf(options.stderr, "Snapshot:\n")
		fmt.Fprintf(options.stderr, "  Binary:     %s\n", config.Binary)
		fmt.Fprintf(options.stderr, "  Keystrokes: %s\n", config.Keystrokes)
		fmt.Fprintf(options.stderr, "  Frame:      %s\n", flags.frame)
		if config.Output != "" {
			fmt.Fprintf(options.stderr, "  Output:     %s\n", config.Output)
		}
		if config.CastOutput != "" {
	...
	if result.GIFPath != "" {
		absOutput, _ := filepath.Abs(result.GIFPath)
		if absOutput == "" {
			absOutput = result.GIFPath
		}

		if flags.copyPath {
			if err := copyToClipboard(absOutput); err != nil && flags.verbosity != "quiet" {
				fmt.Fprintf(options.stderr, "Warning: could not copy to clipboard: %v\n", err)
			} else if flags.verbosity != "quiet" {
				fmt.Fprintf(options.stderr, "Copied path to clipboard\n")
			}
		...
		if flags.openGIF {
			if err := openFile(absOutput); err != nil && flags.verbosity != "quiet" {
				fmt.Fprintf(options.stderr, "Warning: could not open PNG: %v\n", err)
			}
		}
	}
...
func classifyRunError(err error) error {
	...
	case errors.Is(err, pngrenderer.ErrValidation):
		return cliError{code: exitValidation, err: err}
	default:
		return err
	}
